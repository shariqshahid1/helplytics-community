    ...
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <main className="max-w-5xl mx-auto px-4 py-16">
        <div className="text-center mb-20">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white border border-zinc-200 shadow-sm text-zinc-900 text-xs font-bold mb-6">
            <Trophy className="h-3.5 w-3.5 text-yellow-500" />
            <span>Community Hall of Fame</span>
          </div>
          <h1 className="text-6xl font-black tracking-tight text-zinc-900 mb-6">Top Contributors</h1>
        ...
        <div className="space-y-6">
          {leaders.map((leader, index) => (
            <Card 
              key={leader.id} 
              className={`border-none shadow-sm transition-all duration-500 rounded-[40px] overflow-hidden group ${
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="flex items-center gap-8">
                    ...
                    <div className="relative">
                      <div className={`h-24 w-24 rounded-[32px] overflow-hidden border-4 transition-transform group-hover:scale-105 duration-500 ${index === 0 ? 'border-indigo-500/30' : 'border-zinc-50 shadow-sm'}`}>
                        {leader.imageUrl ? (
                          <img src={leader.imageUrl} alt={leader.name} className="h-full w-full object-cover" />
                        ) : (
                          <div className="h-full w-full flex items-center justify-center bg-zinc-100 text-zinc-400 font-bold text-2xl">
                            {leader.name[0]}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="space-y-2">
                      <h3 className={`text-2xl font-black ${index === 0 ? 'text-white' : 'text-zinc-900'}`}>{leader.name}</h3>
                  ...
                  <div className="flex items-center gap-12">
                    <div className="text-center">
                      ...
                      <div className="flex items-center gap-2 justify-center">
                        <Star className={`h-5 w-5 ${index === 0 ? 'text-indigo-400 fill-indigo-400' : 'text-indigo-600 fill-indigo-600'}`} />
                        <span className="text-3xl font-black">{leader.trustScore}</span>
                      </div>
                    </div>
